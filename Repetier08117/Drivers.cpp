...
void LaserDriver::changeIntensity(uint8_t newIntensity)
{
    if(EVENT_SET_LASER(newIntensity))
    {
        ...
        if((Printer::mode == PRINTER_MODE_LASER)&&(!LASER_SAFE))  //sets intensity for next move blocks reinitialisation  
        {
        newIntensity=0; 
        WRITE(LASER_PIN,(LASER_ON_HIGH ? newIntensity > 199 : newIntensity < 200));
        GCode::executeFString(PSTR("M117 LASER SAFETY OFF"));
        } 
        else
        WRITE(LASER_PIN,(LASER_ON_HIGH ? newIntensity > 199 : newIntensity < 200));//keep it here,important for immediate action during move
                                                                                   //necessary for fast switch off from command loop
