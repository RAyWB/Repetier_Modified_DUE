...
bool BLOCK = false; //need for safety door
...
//###########  Safety 
    ...
    if ((Printer::mode == PRINTER_MODE_LASER) && (!LASER_SAFE))
    {
      LaserDriver::changeIntensity(0);// works immediate!!
      Com::printWarningFLN(PSTR("LASER OFF FOR SAFETY"));
      UI_STATUS("LASER OFF FOR SAFETY");
      Printer::setBlockingReceive(true);
      BLOCK = true; // be shure communication is blocked from here
    }
    if (BLOCK && LASER_SAFE) //set blocking back when door closed
    {
   ...
   switch(com->G) {
    ...
    case 0: // G0 
        ...
        if(Printer::setDestinationStepsFromGCode(com)) // For X Y Z E F
                ...
                OLDLaserOn = LaserDriver::laserOn;
                if( Printer::mode == PRINTER_MODE_LASER) 
                  {
                    LaserDriver::laserOn = false;
                    LaserDriver::firstMove = true; //set G1 flag for Laser
                    Com::printWarningFLN(PSTR("firstmove_true"));//for debug
                  }
#endif 
            ...
            LaserDriver::laserOn = OLDLaserOn;//laser on again
#endif
    ...
    case 1: // G1
        ...
        if(Printer::setDestinationStepsFromGCode(com)) // For X Y Z E F
           ...
           if(( Printer::mode == PRINTER_MODE_LASER)  &!(com->hasZ()))
            {
...
//######### M6 ################
    ...
    case 6://Tool change
#if defined(SUPPORT_CNC) && SUPPORT_CNC && SUPPORT_M6
      if (Printer::mode == PRINTER_MODE_CNC) 
      { char buf[35];
        char buf2[35]; 
        if (com->hasT()) 
           ...
           {
            sprintf(buf, "OK to continue");
            sprintf(buf2,"push OK ");
            UI_STATUS(PSTR(buf2));
            Com::printFLN(PSTR(buf));
           }
             Printer::setBlockingReceive(true);
             waitUntilEndOfAllMoves();
             WRITE(CNC_ENABLE_PIN, !CNC_ENABLE_WITH);// switch off for safety in case of forgotten m5 in g-code
        
        while (!CONTINUE)
        /* wait for acknowledge key pressed .no release by uiaction no possibility to release from host
         *  for safety reasons to ensure hands are off and and no second person can start cycle from host
         *  for shure itÂ´s not dummy proof but relatively high level safe
         */
        {
         Printer::defaultLoopActions();
            UI_STATUS(PSTR(buf2));
